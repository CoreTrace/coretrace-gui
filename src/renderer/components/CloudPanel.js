...
class CloudPanel {
  ...
  render() {
    ...
    if (st.status === 'signed_out' || st.status === 'signing_in') {
      ...
      if (st.summary && st.status === 'finished') parts.push(`<div class="cloud-card cloud-summary"><div>${esc(st.summary.conclusion || 'done')} · billed ${esc(st.summary.billedCtu)} CTU · ${esc(st.summary.thresholdCount)} finding(s)${(st.summary.conclusion === 'ctu_cap' || st.summary.conclusion === 'capped') ? ' · stopped at the CTU cap, report truncated' : ''}</div><div class="cloud-hint">Findings are listed in the CTrace results view and marked in the editor.</div><div class="ctrace-btn-row"><button class="ctrace-clear-btn" data-cloud="save-sarif">Save SARIF</button></div></div>`);
