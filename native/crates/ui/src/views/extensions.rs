...
pub fn extensions_panel(state: AppState) -> impl IntoView {
    ...
    let sidecar_status = match ext.sidecar.get().and_then(|s| s.port()) {
        Some(port) => format!("Extension host: running (port {port})"),
        None => "Extension host: starting...".to_string(),
    };

    v_stack((
        label(move || sidecar_status.clone()).style(|s| s.padding(4.0).color(theme::TEXT_MUTED)),
        h_stack((
            theme::text_input(ext.search_query)
                .keyboard_navigable()
                .on_key_down(Key::Named(NamedKey::Enter), |_| true, move |_| ext.search())
                .style(|s| s.flex_grow(1.0)),
            theme::button("Search").action(move || ext.search()),
        ))
...
fn search_result_row(summary: ExtensionSummary, state: AppState) -> impl IntoView {
    ...
    h_stack((
        theme::button("Install").action(move || state.extensions.request_install(install_summary.clone())),
        v_stack((
            label(move || title.clone()),
            label(move || subtitle.clone()).style(|s| s.color(theme::TEXT_MUTED)),
        ))
...
fn installed_row(path: PathBuf, manifest: ExtensionManifest, state: AppState) -> impl IntoView {
    ...
    h_stack((
        theme::button("Uninstall").action(move || state.extensions.uninstall(&uninstall_path)),
        v_stack((
            label(move || title.clone()),
            label(move || subtitle.clone()).style(|s| s.color(theme::TEXT_MUTED)),
        ))
...
fn confirm_dialog_content(summary: ExtensionSummary, state: AppState) -> impl IntoView {
    ...
    v_stack((
        label(move || format!("Install \"{title}\"?")).style(|s| s.font_weight(floem::text::Weight::BOLD)),
        label(move || id.clone()).style(|s| s.color(theme::TEXT_MUTED)),
        label(move || description.clone()),
        h_stack((
            theme::button("Install").action(move || state.extensions.confirm_install()),
            theme::button("Cancel").action(move || state.extensions.cancel_install()),
        ))
        .style(|s| s.column_gap(6.0)),
    ))
    .style(|s| {
        s.padding(10.0)
            .margin(6.0)
